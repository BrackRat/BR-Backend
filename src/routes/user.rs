...
#[get("/me")]
pub(crate) async fn get_user_detail(client: web::Data<PrismaClient>) -> impl Responder {
    ...
    let claims = match result {
        None => {
            return generate_response(ResponseStatus::BadRequest, None, Some("Unauthorized"));
        }
        Some(claims) => {
            claims
        }
